...
import org.example.homework.Point;
...
public class Robot implements Runnable{
    ...
    private Point entryCell;
    private Deque<Point> cellsWhichWillBeVisited;
    ...
    public void addCellsWhichWillBeVisited(int row, int col){
        ...
        //condition for a
        if( row-1 >=0 && col+1 < n ){
            this.cellsWhichWillBeVisited.addLast(new Point(row-1,col+1));

        }
        //condition for b
        if ( col+1 < n){
            this.cellsWhichWillBeVisited.addLast(new Point(row,col+1));
        }
        //condition for c
        if ( row+1 < n && col+1 < n ){
            this.cellsWhichWillBeVisited.addLast(new Point(row+1,col+1));
        ...
        //condition for d
        if( row+1 < n){
            this.cellsWhichWillBeVisited.addLast(new Point(row+1,col));
        }
        //condition for e
        if( row+1 < n && col-1 >=0 ){
            this.cellsWhichWillBeVisited.addLast(new Point(row+1,col-1));
    ...
    private boolean setRandomEntryPoint(){
        ...
        this.entryCell = new Point(row,col);
